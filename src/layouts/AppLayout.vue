...
const navItems = [
  ...
  },
  {
    ...
    to: '/tipos-de-imoveis',
    clientOnly: true,
    icon: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 21h19.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21" /></svg>`,
  },
  {
    label: 'Leads',
    to: '/leads',
  ...
  },
  {
    ...
    clientOnly: true,
    section: 'settings',
    icon: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 4.5h16.5m-16.5 0A2.25 2.25 0 001.5 6.75v10.5A2.25 2.25 0 003.75 19.5h16.5a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0020.25 4.5m-16.5 0l2.25 3h12l2.25-3M9 12h6m-6 3h3" /></svg>`,
  },
  {
    label: 'Equipe',
    to: '/equipe',
...
const visibleNavItems = computed(() =>
  navItems.filter(item => (!item.internalOnly || isInternalStaff.value) && (!item.clientOnly || !isInternalStaff.value))
)
  ...
  <div class="min-h-screen bg-[#f2f4f8] dark:bg-slate-900 flex">
    ...
    >
      ...
      <nav class="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto">
        ...
        <template v-if="settingsNavItems.length > 0">
          <p v-if="!collapsed" class="px-3 mt-3 mb-2 text-[10px] font-bold tracking-[0.18em] uppercase text-slate-400 dark:text-slate-500">
            Configurações
          </p>
          <template v-for="item in settingsNavItems" :key="item.to">
            <component
              :is="item.soon ? 'div' : RouterLink"
              v-bind="item.soon ? {} : { to: item.to }"
              class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 group"
