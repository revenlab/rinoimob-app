        ...
        <template v-else>
            <div v-if="loadingDashboard" class="space-y-6">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
                    <div v-for="index in 4" :key="index" class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow">
                        <div class="h-3 w-24 animate-pulse rounded bg-slate-200 dark:bg-slate-700 mb-4"></div>
                        <div class="h-9 w-20 animate-pulse rounded bg-slate-200 dark:bg-slate-700 mb-3"></div>
                        <div class="h-3 w-28 animate-pulse rounded bg-slate-200 dark:bg-slate-700"></div>
                    </div>
                </div>
                <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
                    <div class="xl:col-span-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow">
                        <div class="h-4 w-40 animate-pulse rounded bg-slate-200 dark:bg-slate-700 mb-5"></div>
                        <div class="space-y-4">
                            <div v-for="index in 4" :key="index" class="flex items-center gap-3">
                                ...
                                <div class="flex-1 space-y-2">
                                    <div class="h-3 w-40 animate-pulse rounded bg-slate-200 dark:bg-slate-700"></div>
                    ...
                    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow">
                        <div class="flex flex-col items-center">
                            <div class="h-16 w-16 animate-pulse rounded-full bg-slate-200 dark:bg-slate-700 mb-4"></div>
                            <div class="h-4 w-36 animate-pulse rounded bg-slate-200 dark:bg-slate-700 mb-2"></div>
                            <div class="h-3 w-40 animate-pulse rounded bg-slate-200 dark:bg-slate-700 mb-6"></div>
                        </div>
                        <div class="space-y-3">
                            <div v-for="index in 3" :key="index" class="h-4 animate-pulse rounded bg-slate-200 dark:bg-slate-700"></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-else class="space-y-6">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
                    <RouterLink to="/tenants" class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow hover:shadow-md transition-shadow">
                        <p class="text-xs font-semibold tracking-[0.15em] uppercase text-slate-400 mb-2">Total Tenants</p>
                        <p class="text-3xl font-bold text-indigo-600 dark:text-indigo-400">{{ supportDashboard?.totalTenants ?? 0 }}</p>
                        <p class="text-xs text-slate-400 mt-1">Ver tenants cadastrados</p>
                    </RouterLink>

                    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow">
                        <p class="text-xs font-semibold tracking-[0.15em] uppercase text-slate-400 mb-2">Tenants Ativos</p>
                        <p class="text-3xl font-bold text-emerald-600 dark:text-emerald-400">{{ supportDashboard?.activeTenants ?? 0 }}</p>
                        <p class="text-xs text-slate-400 mt-1">Operando normalmente</p>
                    </div>

                    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow">
                        <p class="text-xs font-semibold tracking-[0.15em] uppercase text-slate-400 mb-2">Tenants Inativos</p>
                        <p class="text-3xl font-bold text-amber-500 dark:text-amber-400">{{ supportDashboard?.inactiveTenants ?? 0 }}</p>
                        <p class="text-xs text-slate-400 mt-1">Demandam atenção</p>
                    </div>

                    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow">
                        <p class="text-xs font-semibold tracking-[0.15em] uppercase text-slate-400 mb-2">Total Usuários</p>
                        <p class="text-3xl font-bold text-violet-600 dark:text-violet-400">{{ supportDashboard?.totalUsers ?? 0 }}</p>
                        <p class="text-xs text-slate-400 mt-1">clientes no sistema</p>
                    </div>
                </div>

                <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
                    <div class="xl:col-span-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow flex flex-col">
                        <div class="flex items-center justify-between mb-4">
                            <p class="text-xs font-semibold tracking-[0.15em] uppercase text-slate-400">Atividade Recente do Agente</p>
                            <RouterLink to="/tenants" class="text-xs text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400">
                                Ver auditoria completa →
                            </RouterLink>
                        </div>

                        <div class="flex-1 space-y-4">
                            <template v-if="(supportDashboard?.recentActivity?.length ?? 0) > 0">
                                <div
                                    v-for="log in supportDashboard?.recentActivity"
                                    :key="log.id"
                                    class="flex items-center gap-3"
                                >
                                    <span class="h-2.5 w-2.5 rounded-full flex-shrink-0" :class="supportActivityDotClass(log.action)"></span>
                                    <div class="min-w-0 flex-1">
                                        <p class="text-sm font-medium text-slate-800 dark:text-slate-200 truncate">{{ formatAuditAction(log.action) }}</p>
                                        <p class="text-xs text-slate-400 truncate">{{ timeAgo(log.createdAt) }}</p>
                                    </div>
                                </div>
                            </template>
                            <p v-else class="text-sm text-slate-400 text-center py-8">Nenhuma atividade recente</p>
                        </div>
                    </div>

                    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow flex flex-col">
                        <p class="text-xs font-semibold tracking-[0.15em] uppercase text-slate-400 mb-4">Perfil do Agente</p>
                        ...
                        <div class="flex-1 space-y-3 text-sm">
                            <div class="flex items-center justify-between gap-3">
                                <span class="text-slate-500">Papel interno</span>
                                <span class="font-medium text-slate-800 dark:text-slate-200 text-right">{{ systemRoleLabel(authStore.currentUser?.systemRole) }}</span>
                            </div>
                            <div class="flex items-center justify-between gap-3">
                                <span class="text-slate-500">Status</span>
                                <span
                            ...
                            <div class="flex items-center justify-between gap-3">
                                <span class="text-slate-500">Membro desde</span>
                                <span class="font-medium text-slate-800 dark:text-slate-200 text-right">{{ formatDate(authStore.currentUser?.createdAt) }}</span>
                            </div>
                        </div>

                        <RouterLink
                            to="/profile"
                            class="w-full mt-4 h-10 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 rounded-xl text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors flex items-center justify-center"
                        >
                            Editar perfil
                        </RouterLink>
                    </div>
...
import { ArrowPathIcon, ChatBubbleLeftRightIcon, CheckCircleIcon, PlusIcon } from '@heroicons/vue/20/solid'
...
import type { SupportDashboard } from '@/types/tenantsAdmin'
...
const AUDIT_ACTION_LABELS: Record<string, string> = {
    ...
    SUPPORT_VIEW_DASHBOARD: 'Visualizou dashboard',
    TENANT_ACTIVATED: 'Ativou tenant',
    TENANT_DEACTIVATED: 'Desativou tenant',
    USER_ACTIVATED: 'Ativou usuário',
    USER_DEACTIVATED: 'Desativou usuário',
    SUPPORT_ROLE_CHANGED: 'Alterou papel interno',
    SUPPORT_PERMISSIONS_UPDATED: 'Atualizou permissões',
    SUPPORT_RESEND_INVITATION: 'Reenviou convite',
    SUPPORT_RESET_ACCESS: 'Resetou acesso',
}
...
const supportDashboard = ref<SupportDashboard | null>(null)
...
function supportActivityDotClass(action: string) {
    if (action.includes('ACTIVATED')) return 'bg-emerald-500'
    if (action.includes('DEACTIVATED')) return 'bg-amber-500'
    if (action.includes('RESET') || action.includes('INVITATION')) return 'bg-violet-500'
    if (action.includes('ROLE') || action.includes('PERMISSIONS')) return 'bg-indigo-500'
    return 'bg-blue-500'
}
...
onMounted(async () => {
    if (authStore.isInternalStaff) {
        try {
            ...
            supportDashboard.value = await tenantsAdminService.getDashboard()
        } catch {
        } finally {
    ...
    try {
        const response = await taskService.list({ pending: true, size: 3 })
        pendingTasks.value = response.content
    } catch {
    }
    ...
    try {
        ...
        for (const lead of page.content) {
            const events = await leadService.getEvents(lead.id)
            events.forEach(event => allEvents.push({ ...event, leadName: lead.name }))
        }
